       ...
       // }
        ...
        stage('Install') {
         ...
         stage('Stage-9 : Deployment - Deploy a Artifact devops-3.0.0-SNAPSHOT.war file to Tomcat Server') { 
            steps {
                sh 'curl -u admin:redhat@123 -T target/**.war "http://35.154.48.169:8080/manager/text/deploy?path=/project&update=true"'
            }
        } 
  
          stage('Stage-10 : SmokeTest') { 
            steps {
                sh 'curl --retry-delay 10 --retry 5 "http://35.154.48.169:8080/project"'
            }
        }
        }
